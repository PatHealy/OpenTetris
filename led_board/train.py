...
class LEDTrainer:
    ...
    def train(self):
        ...
        checkpoint_dir = os.listdir("led_board/checkpoints/")
        ...
        # if directory is not empty
            ...
            pop = neat.Checkpointer().restore_checkpoint("led_board/checkpoints/neat-checkpoint-" + str(checkpoint))
        ...
        pop.add_reporter(neat.Checkpointer(generation_interval=1, time_interval_seconds=1200,
                                           filename_prefix='led_board/checkpoints/neat-checkpoint-'))
        ...
        with open("./led_board/winner.pickle", 'wb') as model_file:
            # save the model
        ...
        while len(models) > 0:
            ...
            for t, m, g in zip(tetrises, models, gen):
                ...
                if g.fitness > self.max_fitness:
                    ...
                    max_fit_model_dir = os.listdir("./led_board/max_fit_model/")
                    # iterate through each file
                    for file_name in max_fit_model_dir:
                        # delete each file
                        os.remove("./led_board/max_fit_model/" + file_name)

                    # create a file for global high scorer model
                    with open("./led_board/max_fit_model/max_fit_model_" + str(t.get_score()) + ".pickle", 'wb') as model_file:
                        # save the model
